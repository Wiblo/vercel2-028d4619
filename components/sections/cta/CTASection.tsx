...
import { ctaContent } from "@/lib/data/cta"
...
/**
 * CTA (Call-to-Action) section with background image and content card
 * Features a backdrop-blurred card overlay on a full-width background image
 * Content is pulled from lib/data/cta.ts
 * Width matches the hero section for visual consistency
 */
export function CTASection({ className }: CTASectionProps) {
  const { title, description, ctaText, ctaUrl, backgroundImage, backgroundImageAlt } = ctaContent

  return (
    <SectionWrapper className={cn("bg-background", className)}>
      <div className="relative mx-auto min-h-[600px] max-h-screen bg-transparent max-w-7xl xl:max-w-none 2xl:px-48 p-2 md:max-h-full">
